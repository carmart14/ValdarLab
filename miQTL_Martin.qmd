...
sigs <- tibble(
  "locus" = numeric(length = length(locus_matrix)),
  "intercept" = numeric(length = length(locus_matrix))
)


for (ii in 1:length(locus_matrix)) {
  locus <- reader$getFirstLocus(chrom = 19)
  locus_matrix <- reader$getLocusMatrix(locus = locus, subjects = allRNA$STRAIN, model = "additive")
  model <- lm( Y ~ 1 + locus_matrix, data = general)
  
  sigs[ii, "intercept"] <- model$coefficients[1:9]
