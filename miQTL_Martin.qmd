...
miqtl:: plot_locus.effect.from.scan(scan.object = scan, locus = "UNC30286399") 
...
sigRNA |> 
  ...
  geom_vline(xintercept = 38.203233, col="red", linetype="longdash", size =1)+
  labs(title = "y ~ 1 + Locus Matrix", 
       x = "Position(Mb)",
...
colMeans(results.biallelic$post.hap.effects)
colMeans(results.biallelic$prior.D$P)
...
prior.M.uniform <- list(model.type = "uniform", M.IDs = "0,0,0,0,0,0,0,0")
...
colMeans(results.crp$post.hap.effects)
results.crp$post.hap.effects[1:10,]



# using the mcv.data priors liat instead of the fixed effect  
result_test <- TIMBR(exp, prior.D = prior.D, prior.M = mcv.data$prior.M$list, verbose = F)  
as.matrix(head(result_test$p.M.given.y, 10))
TIMBR.plot.haplotypes(result_test, TIMBR.output.bkgrd = results1)
...
# not sure how to use, check back later
...
#check back, not sure if this is right
prior.M.bi.list <- ewenss.calc(8, prior.alpha = list(type="fixed", alpha=1))
prior.number.alleles.bi <- exp(tapply(prior.M.bi.list$ln.probs, number.alleles, 
                                       matrixStats::logSumExp))

# Prior Allele Freq Plots
...
lines(1:8, prior.number.alleles.bi, lty = 5, col = "purple")
points(1:8, prior.number.alleles.bi, pch=22, bg="purple")

legend(1, legend=c("Uniform", "CRP","Tri"), col=c("darkcyan", "orange", "purple"), lty=5)


#Posterior Plots

plot(1:8,results.crp$p.K.given.y, type = "l", lty=5, col="white",
  ylim=c(0,1), ylab = "Posterior Probability", xlab= "Number of Alleles",
  main="Posterior Distribution of Alleles")+
  axis(2, at= seq(0, 0.8, by=0.2))+
lines(1:8, results.crp$p.K.given.y, lty=5, col="orange")
points(1:8, results.crp$p.K.given.y, pch=22, bg="orange")
lines(1:8, result_test$p.K.given.y, lty=5, col="darkcyan")
points(1:8, result_test$p.K.given.y, pch=22, bg="darkcyan")
lines(2:8, results.uniform$p.K.given.y, lty=5, col="purple")
points(2:8, results.uniform$p.K.given.y, pch=22, bg="purple")
legend(1, legend = c("CRP", "Test_List", "Uniform"), col = c("orange", "darkcyan", "purple"), lty=5)


# Approximating Bayes Factors for Alternative Prior Distributions
...
library(bmediatR)
library(qtl2)
source("bmediatr_manuscript_support_functions.R") # need to have downloaded to find source location

vignette("bmediatR_in_DO")

# CC colors
cc_col <- as.character(qtl2::CCcolors)

## Downloaded data from the Tutorial manually, the code was running into problems
