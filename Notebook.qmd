...
One on one with Valdar, getting the bigs of the code worked out. Loop is "working" but the main iterater is the wrong thing, so I'll need to change it so the loci is the part that's changing, not the strain. Also there should be a new genome matrix everytime so that will add to the results and help them make sense. 
